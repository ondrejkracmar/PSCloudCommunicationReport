...
Set-PSFConfig -Module 'PSCloudCommunicationReport' -Name 'Example.Setting' -Value 10 -Initialize -Validation 'integer' -Handler { } -Description "Example configuration setting. Your module can then use the setting using 'Get-PSFConfigValue'"
#>
...
Set-PSFConfig -Module $script:ModuleName -Name 'Settings.GraphApiQuery.Select.PstnCalls' -Value @('id','callId','userId','userPrincipalName','userDisplayName','startDateTime','endDateTime','duration','charge','callType','currency','calleeNumber','usageCountryCode','tenantCountryCode','connectionCharge','callerNumber','destinationContext','destinationName','conferenceId','licenseCapability','inventoryType','operator','callDurationSource') -Initialize -Validation 'stringarray' -Description "Specifies filter select parameter to return a set of properties that are different than the default set for an individual resource or a collection of resources."
Set-PSFConfig -Module $script:ModuleName -Name 'Settings.GraphApiQuery.Select.DirectRoutingCall' -Value @('id','correlationId','userId','userPrincipalName','userDisplayName','startDateTime','inviteDateTime','failureDateTime','endDateTime','duration','callType','successfulCall','callerNumber','calleeNumber','mediaPathLocation','signalingLocation','finalSipCode','callEndSubReason','finalSipCodePhrase','trunkFullyQualifiedDomainName','mediaBypassEnabled') -Initialize -Validation 'stringarray' -Description "Specifies filter select parameter to return a set of properties that are different than the default set for an individual resource or a collection of resources."