...
$parentModule = 'PSCloudCommunicationReport'
if (-not $ModuleName) { $ModuleName = 'PSCloudCommunicationReport.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PSCloudCommunicationReport'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PSCloudCommunicationReport.psd1"
$prereqHash = @{
