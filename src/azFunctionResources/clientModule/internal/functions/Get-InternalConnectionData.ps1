...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'PSCloudCommunicationReport.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'PSCloudCommunicationReport.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'PSCloudCommunicationReport.Client.ProtectedToken'
